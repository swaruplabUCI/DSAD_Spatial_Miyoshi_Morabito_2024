...
pdf(paste0(fig_dir, 'hex_annotation_human.pdf'), width=12, height=12)
patch
dev.off()
...
pdf(paste0(fig_dir, 'hex_ananotation_5x_paper.pdf'), width=14, height=16)
patch
dev.off()
...
pdf(paste0(fig_dir, cur_gene, '_featureplot.pdf'), width=20, height=10)
p
dev.off()
...
setwd('/dfs7/swaruplab/smorabit/analysis/ADDS_2021/Picasso2/picasso')

fig_dir <- 'figures/'

plot_df <- read.csv(file='/dfs7/swaruplab/smorabit/analysis/ADDS_2021/Picasso2/picasso/data/cell_meta_vivek2.csv')

# groups <- unique(plot_df$annotation)
# plot_df$annotation <- factor(
#   as.character(plot_df$annotation),
#   levels = sample(groups, length(groups))
# )

p <- ggplot(plot_df, aes(x=vivek1, y=vivek2, color=annotation)) +
...
pdf(paste0(fig_dir, 'ggplot_vivek.pdf'), width=8, height=10)
p
dev.off()
...
pdf(paste0(fig_dir, 'ggplot_vivek_legend.pdf'), width=12, height=10)
p
dev.off()
