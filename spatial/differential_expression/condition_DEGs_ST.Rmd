...
Figure out which visium samples are in the 
...
seurat_human$ID <- paste0(as.character(seurat_human$Slide), '_', as.character(seurat_human$CaptureArea))

tiff_sample <- 'V10D07-125_D1'
tiff_sample %in% unique(seurat_human$ID)

cur_seurat_meta <- subset(seurat_human@meta.data, ID == tiff_sample)


sample_meta <- read.csv(meta_file, stringsAsFactors=FALSE)

p1 <- cur_seurat_meta %>% 
  ggplot(aes(x=imagecol, y=-imagerow, color=annotation)) + 
  geom_point() + 
  coord_fixed()

pdf(paste0(fig_dir, 'test_spatial_anno.pdf'), width=7, height=7)
p
dev.off()
...
seurat_5x$ID <- paste0(as.character(seurat_5x$SlideSerial), '_', as.character(seurat_5x$CaptureArea))

tiff_sample <- 'V11A13-399_A1'
tiff_sample %in% unique(seurat_5x$ID)

cur_seurat_meta <- subset(seurat_5x@meta.data, ID == tiff_sample)
...
mg1_markers <- subset(markers, group == 'MG1') %>% 
  slice_max(order_by = avg_log2FC, n=50) %>% .$gene

mg2_markers <- subset(markers, group == 'MG2') %>% 
  slice_max(order_by = avg_log2FC, n=50) %>% .$gene

setdiff(mg1_markers, mg2_markers)
setdiff(mg2_markers, mg1_markers)



asc1_markers <- subset(markers, group == 'ASC1') %>% 
  slice_max(order_by = avg_log2FC, n=50) %>% .$gene

asc2_markers <- subset(markers, group == 'ASC2') %>% 
  slice_max(order_by = avg_log2FC, n=50) %>% .$gene

asc4_markers <- subset(markers, group == 'ASC4') %>% 
  slice_max(order_by = avg_log2FC, n=50) %>% .$gene


setdiff(asc1_markers, asc2_markers)
setdiff(asc2_markers, asc1_markers)
setdiff(asc4_markers, c(asc1_markers, asc2_markers))

tp_markers <- subset(markers, group == 'T-Pericyte') %>% 
  slice_max(order_by = avg_log2FC, n=50) %>% .$gene

mp_markers <- subset(markers, group == 'M-Pericyte') %>% 
  slice_max(order_by = avg_log2FC, n=50) %>% .$gene

setdiff(tp_markers, mp_markers)
setdiff(mp_markers, tp_markers)
...
"Deconvolute" which spatial DEGs came from which cell type 
...
for(cur_group in groups){
  ...
  gene_list[[paste0(cur_group, '_up')]] <- unique(cur_markers$gene)
  ...
  gene_list[[paste0(cur_group, '_down')]] <- unique(cur_markers$gene)
...
odc_markers <- subset(markers, group %in% c('ODC1', 'ODC2', 'ODC3')) %>% .$gene %>% unique 
gene_list_ADDS <- gene_list
gene_list_eAD <- gene_list

eAD_down <- unique(as.character(unlist(gene_list_eAD[grepl('down', names(gene_list_eAD))])))
eAD_down <- eAD_down[eAD_down %in% odc_markers]
ADDS_up <- unique(as.character(unlist(gene_list_ADDS[grepl('up', names(gene_list_ADDS))])))
ADDS_up <- ADDS_up[ADDS_up %in% odc_markers]

intersect(eAD_down, ADDS_up)
